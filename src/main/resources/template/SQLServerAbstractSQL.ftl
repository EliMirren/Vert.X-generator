...
public abstract class AbstractSQL<T> {
	...
	public SqlAndParams selectAll(SqlAssist assist) {// 如果Assist为空返回默认默认查询语句,反则根据Assist生成语句sql语句
		if (assist == null) {
			...
			if (assist.getRowSize() == null && assist.getStartRow() == null) {
				...
				String sql = String.format("select * from ( select %s ROW_NUMBER () over (%s) AS page , %s from %s %s ) result %s", distinct, order, column, tableName(), cond, limit);
	...
	public SqlAndParams selectByObj(T obj, String resultColumns, boolean single) {
		StringBuilder sql = new StringBuilder(String.format("select %s %s from %s ", single == true ? " top 1 " : "", resultColumns == null ? columns() : resultColumns, tableName()));
		JsonArray params = null;
