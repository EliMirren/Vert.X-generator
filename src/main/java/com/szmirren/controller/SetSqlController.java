...
public class SetSqlController extends BaseController {
	 ...
	 * @return
	 */
	public SqlConfig getConfig(String name) {
		LOG.debug("执行从数据库中获取配置文件...");
		try {
			SqlConfig config = ConfigUtil.getSQLConfig(name);
		...
		return new SqlConfig().initDefaultValue();
	}
	 ...
	 * @return
	 */
	public SqlConfig getThisConfig() {
		LOG.debug("执行获取当前页面配置文件...");
		SqlConfig config = new SqlConfig(tblPropertyValues, cboTemplate.getValue(), chkOverrideFile.isSelected());
	 ...
	 * @param config
	 */
	public void loadConfig(SqlConfig config) {
